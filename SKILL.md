...
3. **Always end with a risk line.** 技术分析 not 投资建议 — this describes the SCRIPT's
   output (it never auto-executes an order), not a ban on you sharing your own read
   when the user directly asks "你怎么看/给建议". If core data is missing, `建议(主策略)`
   becomes `NO_TRADE·数据不足(缺：...)` instead of a fabricated call — relay that
   verbatim too, don't invent a direction to fill the gap. Every scenario carries a
...
## 回测(可选)
`scripts/crypto/backtest.py` / `scripts/ashare/backtest.py`：喂一份本地OHLC决策日志(CSV/JSON/JSONL)，
复盘价格-only策略历史表现(次K开盘价成交，无未来函数)。是独立的执行层实验，不是`analyze.py`实时信号
的一部分，常规分析不需要碰它。
