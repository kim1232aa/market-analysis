...
# 多周期共振：dir=None(数据/指标不足) 与 dir=0(有数据但震荡) 严格区分,
# 否则数据缺失会被误当成"中性"信号计入共振方向统计。
...
for label, klt in LADDER:
    ...
    if m5 is None or m20 is None or r is None:
        mtf.append((label, None, r)); continue
    d = 1 if c[-1] > m5 > m20 else -1 if c[-1] < m5 < m20 else 0
    mtf.append((label, d, r))
dirs = [d for _, d, _ in mtf if d is not None]
rs = sum(dirs)
if len(dirs) < len(LADDER):
    reson = f"⚪数据不完整·不判定共振(缺{len(LADDER)-len(dirs)}个周期)"
elif all(d > 0 for d in dirs):   reson = "🟢多头共振(各周期同向向上·高信心)"
elif all(d < 0 for d in dirs): reson = "🔴空头共振(各周期同向向下·高信心)"
...
quality = ac.assess_data_quality(rt, tech, mtf, ff, errors=errors, lhb=lhb)
...
if quality["status"] == "NO_TRADE":
    # 核心数据(行情/日K/多周期/主力资金流)缺失: 不据此合成方向性建议,
    # 上面的关键位/情景剧本仍是真实结构数字,仅标注为参考。
...
print("╚═══ 报告块结束 · 以上须完整输出,尤其【面板表】【主力资金流】【合并结论/建议】不可省略 ═══╝")
...
OUT = {"code": rt["code"], "name": rt["name"], "realtime": rt, "tech": tech,
       ...
       "resonance": reson, "bias_score": total, "bias": bias,
