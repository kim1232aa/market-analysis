...
print(f"\n{src}")
_sig = hashlib.sha1(f"{rt['code']}{rt['price']}{ff_today}{total}".encode()).hexdigest()[:8]
print(f"🔒校验 {rt['code']} last={q(rt['price'])} 资金流={('%+.2f亿'%(ff_today/1e8)) if ff_today is not None else '缺'} 评分={total} sig={_sig}")
print("⚠️ 非投资建议·A股T+1&涨跌停板·务必控制仓位。数据为最新快照(盘中实时/盘后为当日收盘)。位与R:R为结构自动测算,需按盘微调。")
print("╚═══ 报告块结束 · 须完整输出(面板表/主力资金流/合并结论/建议/🔒校验行不可删) · 🔒校验行=跑脚本的证据,缺=假报告(JSON段可不贴) ═══╝")
